...
fn parse_field(data: &[u8]) -> (Field, &[u8]) {
    debug_println!("considering {data:#04x?}");
    let (tag, remainder) = parse_varint(data);
    let (field_num, wire_type) = unpack_tag(tag);
    debug_println!("field_num {field_num}, wire_type {wire_type:?}");
    let (fieldvalue, remainder) = match wire_type {
        ...
        WireType::Len => {
            let (value, remainder) = parse_varint(remainder);
            debug_println!("len {value}, remainder {remainder:#04x?}");
            let s = remainder.get(..value as usize).unwrap();
            let remainder = remainder.get(value as usize..).unwrap();
            debug_println!(
                "string {}, remainder {remainder:#04x?}",
                String::from_utf8_lossy(s)
            );
            (FieldValue::Len(s), remainder)
        }
        WireType::I32 => {
            let (value, remainder) = parse_varint(remainder);
            (FieldValue::I32(i32::try_from(value).unwrap()), remainder)
        }
    };
    (
        Field {
...
impl<'a> ProtoMessage<'a> for Person<'a> {
    fn add_field(&mut self, field: Field<'a>) {
        match field.field_num {
            ...
            n => panic!("unexpected field number {n}"),
        }
    }
}
